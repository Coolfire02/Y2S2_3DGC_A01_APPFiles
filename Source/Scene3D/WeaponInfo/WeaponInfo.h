...
#include "../Items/Items/Item.h"

class CWeaponInfo : public Item
{
public:
	// Default Constructor
	CWeaponInfo(void);
	...
	virtual bool Discharge(glm::vec3 vec3Position, glm::vec3 vec3Front, glm::vec3 vec3Right, glm::vec3 vec3Up, CSolidObject* pSource = NULL);
	// Reload this weapon
...
protected:
	...
	CCrossHair::CROSSHAIR_TYPE crosshair;

	bool aimDownSights;

	bool automaticShooting;

	float defaultBulletDamage;
	...
	//SPRAY
	float xSpray, ySpray;
	float xSprayOffset, ySprayOffset;

	float xSprayMax, ySprayMax;

	float xSprayLinear, ySprayLinear;
	float xSprayAcceleration;
	float ySprayAcceleration;

	float sprayCooloffThreshold;

	//RECOIL
	float xRecoilMax, yRecoilMax;
	float xRecoilAcceleration, yRecoilAcceleration;
