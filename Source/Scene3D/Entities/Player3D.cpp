...
bool CPlayer3D::Init(void)
{
	...
	/*mesh = CMeshBuilder::GenerateBox(glm::vec4(1.0f, 1.0f, 1.0f, 1.0f),
		1.0f, 1.0f, 1.0f);*/
	mesh = CMeshBuilder::GenerateBox(glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), vec3Scale.x * 2, vec3Scale.y * 2, vec3Scale.z * 2);
	...
	vec3Position.y = cTerrain->GetHeight(vec3Position.x, vec3Position.z) + vec3Scale.y;
...
{
	return glm::lookAt(vec3Position, vec3Position + vec3Front + cCamera->vec3FrontOffset, vec3Up);
}
...
void CPlayer3D::AttachCamera(CCamera* cCamera)
{
	...
	if (cCamera)
	{
		...
		cCamera->vec3Front = vec3Front + cCamera->vec3FrontOffset;
...
void CPlayer3D::ProcessStanceChange(PLAYER_STANCE newStance)
{
	if (newStance != stance)
	{
		switch (newStance)
		...
		case RUNNING:
			fMovementSpeed = 3.f;
			cCamera->vec3Offset = glm::vec3(0, 0, 0);
			break;
		case WALKING:
			fMovementSpeed = 1.2f;
			cCamera->vec3Offset = glm::vec3(0, 0, 0);
		...
		case CROUCHING:
			fMovementSpeed = 0.9f;
			cCamera->vec3Offset = vec3Up * -0.08f;
			break;
		}
...
CPlayer3D::PLAYER_STANCE CPlayer3D::GetPlayerStance()
{
	return stance;
}
 ...
 */
bool CPlayer3D::DischargeWeapon(bool spray) const
{
	
	if ((iCurrentWeapon == 0) && (cPrimaryWeapon))
	{
		if (spray && !cPrimaryWeapon->IsSprayType())
			return false;
		return cPrimaryWeapon->Discharge(vec3Position + cCamera->vec3Offset, vec3Front , vec3Right, vec3Up, (CSolidObject*)this);
	}
	else if ((iCurrentWeapon == 1) && (cSecondaryWeapon))
	{
		if (spray && !cSecondaryWeapon->IsSprayType())
			return false;
		return cSecondaryWeapon->Discharge(vec3Position + cCamera->vec3Offset, vec3Front , vec3Right, vec3Up, (CSolidObject*)this);
	}
	//return NULL;
}
...
{
	if (cPhysics3D.GetStatus() == CPhysics3D::STATUS::IDLE && stance != CROUCHING)
	{
		cPhysics3D.SetStatus(CPhysics3D::STATUS::JUMP);
		cPhysics3D.SetInitialVelocity(glm::vec3(0.0f, 0.65f, 0.0f));
	}
}

bool CPlayer3D::IsInAir()
{
	return cPhysics3D.GetStatus() == CPhysics3D::STATUS::JUMP;
}
...
void CPlayer3D::ProcessMovement(const PLAYERMOVEMENT direction, const float deltaTime)
{
	...
	glm::vec3 toAdd(0,0,0);

	if (direction == PLAYERMOVEMENT::FORWARD)
		toAdd += vec3Front * velocity;
	if (direction == PLAYERMOVEMENT::BACKWARD)
		toAdd -= vec3Front * velocity;
	if (direction == PLAYERMOVEMENT::LEFT)
		toAdd -= vec3Right * velocity;
	if (direction == PLAYERMOVEMENT::RIGHT)
		toAdd += vec3Right * velocity;

	if(toAdd.y > 1) toAdd.y = 1 ;

	glm::vec3 pos = vec3Position + toAdd;
	cTerrain->GetHeight(pos.x, pos.z);
	float heightDiff = cTerrain->GetHeight(pos.x, pos.z) - YHeightAtLocation(vec3Position);
	if (heightDiff > 0)
	{
		float gradient = heightDiff / (toAdd.length()*deltaTime);
		//cout << "Gradient " << gradient << " Height DIFF" << heightDiff << " to add.length " << toAdd.length() * deltaTime << endl;


		float angle = tan(heightDiff / toAdd.length()) / (2*3.142) * 360;
		//cout << "Angle " << angle;

		
		if (angle > 0.8f) toAdd *= 0.f;
		else toAdd = ((1.f - angle) / 1.f) * toAdd ;
...
bool CPlayer3D::Update(const double dElapsedTime)
{
	...
	//ut << "Player position " << vec3Position.x << " " << vec3Position.y << " " << vec3Position.z << " " << endl;
...
void CPlayer3D::UpdatePlayerVectors(void)
{
	...
	if (cCamera) glm::normalize(front+ cCamera->vec3FrontOffset);
...
void CPlayer3D::Constraint(void)
{
	...
	this->vec3Position.y = YHeightAtLocation(vec3Position);
}
...
void CPlayer3D::UpdateJumpFall(const double dElapsedTime)
{
	...
	else if (cPhysics3D.GetStatus() == CPhysics3D::STATUS::FALL)
	{
		...
		float fCheckHeight = cTerrain->GetHeight(vec3Position.x, vec3Position.z) + vec3Scale.y;
