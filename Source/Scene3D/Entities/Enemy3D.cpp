...
CEnemy3D::CEnemy3D(void)
	...
	, nextMeleeAttackCountdown(3)
	, meleeDamage(0)
	, target(NULL)
	, cTerrain(NULL)
{
	// Set the default position to the origin
	vec3Position = glm::vec3(0.0f, 0, 0.0f);
...
CEnemy3D::~CEnemy3D(void)
{
	...
	if (cPrimaryWeapon)
		cPrimaryWeapon->SetToDelete(true);
	if (cSecondaryWeapon)
		cSecondaryWeapon->SetToDelete(true);
 ...
 */
bool CEnemy3D::Init(CEnemy3D::ENEMY_TYPE type)
{
	...
	vec3Scale.x = 0.4f;
	vec3Scale.z = 0.4f;
	mesh = CMeshBuilder::GenerateBox(glm::vec4(1, 1, 1, 1), vec3Scale.x * 2, vec3Scale.y * 2, vec3Scale.z * 2);
...
bool CEnemy3D::DischargeWeapon(void) const
{
	...
	{
		return cPrimaryWeapon->Discharge(vec3Position, vec3Front, vec3Right, vec3Up, (CSolidObject*)this);
	}
	else if ((iCurrentWeapon == 1) && (cSecondaryWeapon))
	{
		return cSecondaryWeapon->Discharge(vec3Position, vec3Front, vec3Right, vec3Up, (CSolidObject*)this);
	}
	//return NULL;
}
...
bool CEnemy3D::hasMeleeAttack()
{
	return meleeDamage > 0;
}

void CEnemy3D::SetTarget(CSolidObject* target)
{
	this->target = target;
}

void CEnemy3D::setAttackCooldownDuration(float cooldown)
{
	this->meleeAttackCooldown = cooldown;
	this->nextMeleeAttackCountdown = cooldown;
}

void CEnemy3D::setMeleeDamage(float damage)
{
	this->meleeDamage = damage;
}

bool CEnemy3D::tryMeleeAttack(CSolidObject* target)
{
	if (nextMeleeAttackCountdown <= 0)
	{
		nextMeleeAttackCountdown = meleeAttackCooldown;
...
bool CEnemy3D::Update(const double dElapsedTime)
{
	...
	if (nextMeleeAttackCountdown > 0)
		nextMeleeAttackCountdown -= dElapsedTime;
	...
	case FSM::PATROL:
		...
		else if (glm::distance(vec3Position, target->GetPosition()) < fDetectionDistance)
		{
			sCurrentFSM = FSM::ATTACK;
			iFSMCounter = 0;
			if (_DEBUG_FSM == true)
		...
		break;
	case FSM::ATTACK:
		if (glm::distance(vec3Position, target->GetPosition()) < fDetectionDistance)
		{
			vec3Front = glm::normalize((target->GetPosition() - vec3Position));
			UpdateFrontAndYaw();
...
{
	// Get the new height
	float fNewYValue = cTerrain->GetHeight(vec3Position.x, vec3Position.z) + 0.65;
