	...
	for (it = lSolidObject.begin(); it != end; ++it)
	{
												...
												(*it)->GetPosition() + (*it)->boxMax) == true)
		{
			...
			if ((*it)->GetType() == CSolidObject::TYPE::NPC)
			{
				...
				/*if (cSolidObject->GetType() == CSolidObject::TYPE::PROJECTILE)
				{
					CProjectile* projectile = (CProjectile*)(cSolidObject);
					if (projectile->GetSource() != (*it))
					{
						(*it)->ChangeHealth(-projectile->GetBulletDamage());
						cout << "** This entity/structure now has " << (*it)->GetHealth() << " health remaining **" << endl;

						if ((*it)->GetHealth() < 0)
						{
							if ((*it)->GetType() != CEntity3D::PLAYER)
								(*it)->SetToDelete(true);
						}
					}
				}*/
			...
			{
				//Projectile against enties/structures
				/*CProjectile* projectile = (CProjectile*)(*it);
				if (projectile->GetSource() != cSolidObject)
				{
					cSolidObject->ChangeHealth(-projectile->GetBulletDamage());
					cout << "** This entity/structure now has " << cSolidObject->GetHealth() << " health remaining **" << endl;

					if (cSolidObject->GetHealth() < 0)
					{
						if(cSolidObject->GetType() != CEntity3D::PLAYER)
							cSolidObject->SetToDelete(true);
					}
				}*/
				...
				(*it_other)->GetPosition() + (*it_other)->boxMax) == true)
			{
					...
					)
				{
					...
					if (enemy)
					{
						...
						{
							if (enemy->tryMeleeAttack((*it_other)))
							{
								if ((*it_other)->GetHealth() <= 0)
								{
									cout << "** Structure Broken ***" << endl;
									(*it_other)->SetStatus(false);
								}
								cout << "** Collision between Entity damaged Structure ***" << endl;
							}
						}
					}
		...
		for (unsigned int i = 0; i < cProjectileManager->vProjectile.size(); i++)
		{
				...
				(cProjectileManager->vProjectile[i])->GetPosition()))
			{
				...
				else if ((*it)->GetType() == CSolidObject::TYPE::NPC)
				{
					...
					//Projectile against enties/structures
					if (cProjectileManager->vProjectile[i]->GetType() == CSolidObject::TYPE::PROJECTILE)
					{
						CProjectile* projectile = (CProjectile*)(cProjectileManager->vProjectile[i]);
						if (projectile->GetSource() != (*it))
						{
							(*it)->ChangeHealth(-projectile->GetBulletDamage());
							cout << "** This entity/structure now has " << (*it)->GetHealth() << " health remaining **" << endl;
