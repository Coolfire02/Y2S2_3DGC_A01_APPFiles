...
public:
	...
	bool Init(CEnemy3D::ENEMY_TYPE type);
	...
	void SetTarget(CSolidObject* target);

	void setAttackCooldownDuration(float cooldown);
	void setMeleeDamage(float damage);
	bool hasMeleeAttack();
	bool tryMeleeAttack(CSolidObject* target);
...
protected:
	...
	float nextMeleeAttackCountdown;
